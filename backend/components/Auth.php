...
class Auth extends \fast\Auth
{
    ...
    {
        return Yii::$app->session->get('admin.' . $name);
    }
     ...
     * @return array
     */
    public function getSidebar($params = [], $uid = null, $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = AuthRule::find()->where(['status' => 'normal', 'ismenu' => 1])->orderBy('weigh desc')->asArray()->all();
