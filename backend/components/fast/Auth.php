...
//        }
    ...
    public function getGroups($uid)
    {
        ...
        $query = AuthGroupAccess::find()->alias('aga');
        $query->join('LEFT JOIN', AuthGroup::tableName() . ' as ag', 'aga.group_id = ag.id')
            ->select(['aga.uid', 'aga.group_id', 'ag.id', 'ag.pid', 'ag.name', 'ag.rules'])
            ->where(['aga.uid' => $uid, 'ag.status' => 'normal']);

        $user_groups = $query->asArray()->all();
        // 执行查询
...
//                ->select();
    ...
    public function getRuleList($uid)
    {
        ...
        if (2 == $this->config['auth_type']) {
            $rule = Yii::$app->session->get('_rule_list_' . $uid);
            if ($rule) {
                return $rule;
            }
        }
        ...
        $this->rules = AuthRule::find()
            ->select(['id', 'pid', 'condition', 'icon', 'name', 'title', 'ismenu'])
            ->where($where)
            ->asArray()
            ->all();
        //循环规则，判断结果。
        ...
        if (2 == $this->config['auth_type']) {
            //规则列表结果保存到session
            Yii::$app->session->set('_rule_list_' . $uid, $rulelist);
        }
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($user_info[$uid])) {
            $user_info[$uid] = AuthAdmin::getInfoById($uid);
