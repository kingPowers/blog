...
class Label extends Common
{
	...
	public static function labelIdName ($labelid = '',$status = self::ENABLE_STATUS)
	{
		$labelModel = new Label();
		if ($status) $labelModel->where['status'] = $status; 
		$labelList = $labelModel->allList();
		$labelList = $labelModel->classifyLabel($labelList);
	...
	public static function getLabelByIds ($labelids,$condition = '1=1')
	{
		if ($labelids) return [];
		return static::find()->where(['id' => $labelids])->andWhere($condition)->asArray()->all();
	}
}