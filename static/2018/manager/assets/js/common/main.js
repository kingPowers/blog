    ...
    _initLocation :function (){
      ...
      if(defaultSetting){
        var pageId = defaultSetting.pageId,   //页面编号
          ...
          index = _self._getModuleIndex(parseInt(defaultSetting.moduleId));   //附加参数
    ...
    _setPageSelected:function(moduleIndex,pageId,isReload,search){
      ...
      if(module && pageId){
        ...
        var item = module.menu.getSelected(),
          tab = module.tab,
          href = '',
          suffixIndex = -1;//console.log([moduleIndex,pageId]);
        if(item && item.get('id') == pageId){
          href = item.get('href');//console.log(href);
          ...
          href = search ? (subDir + '?' + search) : subDir;//alert(href);
